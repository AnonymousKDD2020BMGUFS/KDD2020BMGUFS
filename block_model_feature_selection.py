# This script implements python methods for paper Block Model Guided Unsupervised Feature Selection. Submission to KDD 2020.
import numpy as np
from numpy import linalg as LA
import torch

def solver_delta_enhanced(Y,F,Ms,beta,delta,MAXITER,step_size):
...
#		Lb_dr = torch.diag(torch.mm(torch.mm(Y.t(),A),Y) + torch.mm(torch.mm(Ybar.t(),X),Ybar) - torch.mm(torch.mm(Ybar.t(),A),Ybar)*2)*2/(torch.norm(A)**2) - torch.diag(torch.mm(torch.mm(Y.t(),A),Y))*2*Lb/(torch.norm(A)**2)
		...
		L_dr = ( Lb_dr/torch.norm(Lb_dr) )*(1.0-beta) + ( Lm_dr/torch.norm(Lm_dr) )*beta
